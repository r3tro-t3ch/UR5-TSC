...
class TSCLF:
    ...
    def V(self, 
        ...
        _delta_x        = np.concatenate([x - x_d,          -delta_q])
        _delta_x_dot    = np.concatenate([xdot - xdot_d,    w - w_d]) 
    ...
    def V_x(self,
        ...
        delta_x     = np.concatenate([x - x_d, -delta_q])
        _x_dot      = np.concatenate([xdot, w])
        _x_dot_d    = np.concatenate([xdot_d, w_d])

        # print("e_dot : ",(_x_dot_d - _x_dot))

        return 2 * delta_x.T @ self.P, 2 * (_x_dot - _x_dot_d).T @ self.Q
